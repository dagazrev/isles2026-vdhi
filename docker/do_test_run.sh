...
CONTAINER_NAME="isles26_baseline_nnunet5fold_container"
...
INVOKE_TIMEOUT_SECONDS=420
# Grand Challenge: ~7 minutes per case on a T4. Keep local invoke budget aligned.
...
setup() {
    ...
    # Force GPU 0 (workspace rule). nvidia-container-toolkit required.
    if command -v nvidia-smi >/dev/null 2>&1; then
        GPU_ARGS="--gpus device=0"
        log "Enabling GPU 0 via --gpus device=0"
    else
        GPU_ARGS=""
        log "nvidia-smi missing — running on CPU" warning
    fi
